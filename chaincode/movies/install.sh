...
#pushd ../../../chaincode/chaincode/$CHAINCODE_NAME
...
#first peer peer0.org1.blocktick.com
peer lifecycle chaincode install ${CHAINCODE_NAME}.tar.gz 

#Actualizar este  valor con el que obtengan al empaquetar el chaincode: movies_1:a1c05f648dd24bd94128913d73486644ad6c351f19c429c4c661444039688299
...
peer lifecycle chaincode approveformyorg --tls --cafile $ORDERER_CA --channelID $CHANNEL_NAME --name $CHAINCODE_NAME --version $CHAINCODE_VERSION --sequence 1 --waitForEvent --signature-policy "OR ('Org1MSP.peer','Org3MSP.peer')" --package-id movies_1:$CC_PACKAGEID
...
peer lifecycle chaincode commit -o orderer.blocktick.com:7050 --tls --cafile $ORDERER_CA  --peerAddresses peer0.org1.blocktick.com:7051 --tlsRootCertFiles /opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/peerOrganizations/org1.blocktick.com/peers/peer0.org1.blocktick.com/tls/ca.crt --channelID $CHANNEL_NAME --name $CHAINCODE_NAME --version $CHAINCODE_VERSION --sequence 1 --signature-policy "OR ('Org1MSP.peer','Org3MSP.peer')"

#2020-09-03 17:39:05.756 UTC [chaincodeCmd] ClientWait -> INFO 046 txid [453ed408b77c198d7159904c94b8d44b4d7633273f200bafc87c5419901883c2] committed with status (ENDORSEMENT_POLICY_FAILURE) at peer0.org1.blocktick.com:7051
#Error: transaction invalidated with status (ENDORSEMENT_POLICY_FAILURE)



#Let Org3 approve the chaincode package.
CORE_PEER_MSPCONFIGPATH=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/peerOrganizations/org3.blocktick.com/users/Admin@org3.blocktick.com/msp  CORE_PEER_ADDRESS=peer0.org3.blocktick.com:7051  CORE_PEER_LOCALMSPID="Org3MSP"  CORE_PEER_TLS_ROOTCERT_FILE=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/peerOrganizations/org3.blocktick.com/peers/peer0.org3.blocktick.com/tls/ca.crt  peer lifecycle chaincode approveformyorg --tls --cafile /opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/ordererOrganizations/blocktick.com/orderers/orderer.blocktick.com/msp/tlscacerts/tlsca.blocktick.com-cert.pem --channelID $CHANNEL_NAME --name $CHAINCODE_NAME --version $CHAINCODE_VERSION --sequence 1 --waitForEvent --signature-policy "OR ('Org1MSP.peer','Org3MSP.peer')" --package-id movies_1:$CC_PACKAGEID
...
#INIT LEDGER
#peer chaincode invoke -o orderer.blocktick.com:7050 --tls --cafile $ORDERER_CA -C  $CHANNEL_NAME  -n $CHAINCODE_NAME -c '{"Args":["InitLedger"]}'
#Org1 invokes set() with key “car01” and value “........”.
peer chaincode invoke -o orderer.blocktick.com:7050 --tls --cafile $ORDERER_CA -C  $CHANNEL_NAME  -n $CHAINCODE_NAME -c '{"Args":["Set","did:3","ricardo","banana"]}'
...
CORE_PEER_MSPCONFIGPATH=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/peerOrganizations/org2.blocktick.com/users/Admin@org2.blocktick.com/msp  CORE_PEER_ADDRESS=peer0.org2.blocktick.com:7051 CORE_PEER_LOCALMSPID="Org2MSP" CORE_PEER_TLS_ROOTCERT_FILE=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/peerOrganizations/org2.blocktick.com/peers/peer0.org2.blocktick.com/tls/ca.crt  peer chaincode invoke -o orderer.blocktick.com:7050 --tls --cafile $ORDERER_CA -C  $CHANNEL_NAME  -n $CHAINCODE_NAME -c '{"Args":["Set","did:4","marianela","avacado"]}'